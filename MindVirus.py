...
class Cable(CustomObject):
    ...
    def specify_parts(self):
        ...
        self.tracer = Tracer(self.start_point_null, self.start_normal_null, self.end_normal_null, self.end_point_null, tracing_mode="objects", spline_type="b-spline")
        self.cable_profile = Circle(radius=1, helper_mode=True)
        self.sweep_nurbs = SweepNurbs(rail=self.tracer, profile=self.cable_profile, name="Cable", outline_only=True, fill_color=BLACK)
        self.parts = [self.start_point_null, self.start_normal_null, self.end_point_null, self.end_normal_null, self.tracer, self.cable_profile, self.sweep_nurbs]

    def specify_parameters(self):
        self.start_growth_parameter = UCompletion(name="Start", default_value=0)
        self.end_growth_parameter = UCompletion(name="End", default_value=1)
        self.parameters += [self.start_growth_parameter, self.end_growth_parameter]
    ...
    def specify_creation(self):
        self.creation_action = XAction(
            Movement(self.sweep_nurbs.creation_parameter, (0, 1), output=(0, 1), part=self.sweep_nurbs),
...
class MindVirusJourney(CustomObject):
    ...
    def specify_parameters(self):
        self.journey_completion_parameter = UCompletion(name="JourneyCompletion", default_value=1)
